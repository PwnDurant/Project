...
//        判断令牌是否为空
        if(!StringUtils.hasLength(token)) {
            log.error("token为空！");
            throw new UserException(ResultCode.TOKEN_IS_EMPTY);
        }
//        开始解析令牌
        ...
        try{
            ...
            throw new UserException(ResultCode.TOKEN_IS_EXPIRED);
        }catch (SignatureException e){
            log.error("token 签名错误,token:{}",token);
            throw new UserException(ResultCode.TOKEN_SIGNATURE_ERROR);
        }catch (Exception e){
            log.error("token 解析失败,token:{}",token);
            throw new UserException(ResultCode.TOKEN_PARSE_ERROR);
        }
    }
